...
<div class="row w-100 h-100 justify-content-center align-content-center">
    <div class="jumbotron mt-3 p-5 col-12 col-md-6">
        ...
        <p class="lead">{{ __('ui.precio') }}: {{$announcement->price}}€
            <strong class="float-right"> {{ __('ui.categoria') }} <a
                    ...
                    class="text-decoration-none">{{$announcement->category->name}}</a></strong></p>
        <hr>
        ...
        <p><a href="{{route('announcement.category',['name'=>$announcement->category->name,'id'=>$announcement->category->id])}}"
                class="btn btn-warning">Comparar con otros: {{$announcement->category->name}}</a></p>
        <p><a href="mailto:{{$announcement->user->email}}" class="btn btn-light">Contactar al vendedor:
                {{$announcement->user->name}}</a></p>
        <p class="mt-3 mb-0 text-right">{{ __('ui.vendedor') }}: <a href="#" class="text-decoration-none"
